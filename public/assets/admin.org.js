...
const root_url = document.querySelector("#root")
...
const search_holder = document.getElementById("search_holder")
...
function open_search_div() {
    ...
    const elementStyle = getComputedStyle(root);
    root.style.setProperty("--sidebarWidth_right", "300px");
    sidebar_right.style.setProperty("right", "0");
...
function active_sidebar_item(element) {
    ...
    search_func_url = element.dataset.search_url
    // off or seach div
    // search_func_url != "" ? open_search_div() : close_search_div()

    if (element.innerHTML == "Search") { 
        search_holder.style.visibility = 'visible' 
        search_holder.style.height = 'fit-content' 
        content_div.classList.add("active")
    }else{
        search_holder.style.visibility = 'hidden' 
        search_holder.style.height = '0px'
...
async function load_page(page_name) {
    try {
        ...
        // console.log(data)
        // history.pushState({}, page_name,root_url.innerHTML+"/admin/" + page_name);
...
function handleInput(keyword) {
    ...
    timerId = setTimeout(() => {
        dynamic_search(keyword, root_url.innerHTML + "/admin/search_in_org")
    }, delay);
}
        ...
        if (data != 'redirect') {
            ...
            if (data.length > 0) {
                data.forEach(element => {
                    create_result_holder(element.event_url, element.name, element.gov_reg_no, element.email)
                    console.log(element)
                });
            } else {
...
function create_result_holder(eventUrl, orgName, gov_reg_no, email) {
    ...
    // add values
    event_name.innerHTML = orgName
    org_name.innerHTML = "Govermenr rg. : " + gov_reg_no
    event_date.innerHTML = "Email : " + email
    ...
    search_result_div.addEventListener("click", () => {
        window.location.href = root_url.innerHTML + "/Event_org?id=" + eventId
    })
    // result_div.appendChild(search_result_div)
}
